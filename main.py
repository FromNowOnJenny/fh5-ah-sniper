...
def main():
    ...
    time.sleep(5)
    sniperscript(args.count, args.x, args.y)
...
def sniperscript(collect_target: int, monitor_x: int, monitor_y: int):

    def enter_ah():
        print("Entering search")
        while getpixelcolor(ah_search_x, ah_search_y) != (255, 0, 134):
            print("waiting for main auction window")
        while getpixelcolor(ah_search_x, ah_search_y) == (255, 0, 134):
        ...
        while getpixelcolor(s_confirm_x, s_confrm_y) != (255, 0, 134):
            print("p4")
            pass
    ...
    def check_for_auction():  # Checks the auction house for an available auction
        while getpixelcolor(rear_window_x, rear_window_y) != (255, 222, 57):  # checks if in auction list
            print("waiting for auction list")
        if getpixelcolor(car_x, car_y) == (52, 23, 53):  # checks if a car is listed
            if getpixelcolor(px_x, px_y) == (247, 247, 247):
                while getpixelcolor(dj_x, dj_y) == (247, 247, 247):
                    print("waiting for bidding menu")
    ...
    def buyout_outcome():
        ...
        while getpixelcolor(buyout_outcome_check_x, buyout_outcome_check_y) != (52, 23, 53):
            print("Waiting for buyout outcome (2nd mysterious check)")
        buyoutsuccessful = getpixelcolor(buyout_x, buyout_y)  # Checks if the buyout was successful
    ...
    s_confrm_y = int(0.677777778 * monitor_y)
    ...
    buyout_outcome_x = int(0.33489583333 * monitor_x)  # w:643 h:442
    buyout_outcome_y = int(0.40925925925 * monitor_y)
    buyout_x = int(0.3328125 * monitor_x)  # w:639 h:460
    ...
    buyout_failed_x = int(0.33020833333 * monitor_x)  # w:634 h:532
